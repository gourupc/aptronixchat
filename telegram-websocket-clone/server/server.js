...
const ADMIN_EMAIL_APP_PASS = process.env.ADMIN_EMAIL_PASS || 'gjyx tkgx fyad tdej';
...
function sendLoginAlertEmail(ip, userAgent) {
  ...
  const mailOptions = {
    ...
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 500px; padding: 20px; border: 1px solid #ddd; border-radius: 10px;">
        ...
        <p style="margin-top: 20px; font-size: 0.8em; color: #999;">This is an automated security alert from your Doremon Messenger server.</p>
      </div>
    `
  };
  ...
  mailTransporter.sendMail(mailOptions, (err, info) => {
    if (err) {
      ...
      console.log('[LOGIN ALERT] Admin notified successfully:', info.messageId);
    }
  });
}
...
app.post('/api/verify-passcode', (req, res) => {
  ...
  const isCorrect = (passcode && passcode.toString().trim().toLowerCase() === 'doremont0805');
