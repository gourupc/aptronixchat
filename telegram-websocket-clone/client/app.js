...
document.addEventListener('DOMContentLoaded', () => {
  // --- Fawesome TV Security Gate (Stealth Mask Portal) ---
  const fawesomeGate = document.getElementById('fawesome-gate');
  const gatePassField = document.getElementById('gate-pass-field');
  const gatePassSubmit = document.getElementById('gate-pass-submit');
  
  const moviePlayerModal = document.getElementById('movie-player-modal');
  const closePlayerBtn = document.getElementById('close-player-btn');
  const movieVideoElement = document.getElementById('movie-video-element');

  // Verify stored session unlock status
  if (sessionStorage.getItem('gate_unlocked') === 'true') {
    if (fawesomeGate) fawesomeGate.classList.add('hidden');
  }

  // Handle password submissions
  const verifyPasscode = () => {
    const password = gatePassField.value;
    if (password === 'doremon' || password === 'doremon2026') {
      sessionStorage.setItem('gate_unlocked', 'true');
      if (fawesomeGate) {
        fawesomeGate.style.transition = 'opacity 0.3s ease';
        fawesomeGate.style.opacity = '0';
        setTimeout(() => {
          fawesomeGate.classList.add('hidden');
        }, 300);
      }
    } else {
      alert('Access Denied. Incorrect parental control code.');
      gatePassField.value = '';
    }
  };

  if (gatePassSubmit) gatePassSubmit.addEventListener('click', verifyPasscode);
  if (gatePassField) {
    gatePassField.addEventListener('keydown', (e) => {
      if (e.key === 'Enter') verifyPasscode();
    });
  }

  // Handle fake movie trailer players!
  document.querySelectorAll('.btn-play-movie').forEach(card => {
    card.addEventListener('click', (e) => {
      e.stopPropagation();
      const videoSrc = card.getAttribute('data-video');
      if (videoSrc && movieVideoElement && moviePlayerModal) {
        movieVideoElement.src = videoSrc;
        moviePlayerModal.classList.remove('hidden');
        movieVideoElement.play().catch(err => console.log('Autoplay blocked:', err.message));
      }
    });
  });
  ...
  if (closePlayerBtn) {
    closePlayerBtn.addEventListener('click', () => {
      if (movieVideoElement && moviePlayerModal) {
        movieVideoElement.pause();
        movieVideoElement.src = '';
  ...
  if (moviePlayerModal) {
    moviePlayerModal.addEventListener('click', (e) => {
      if (e.target === moviePlayerModal && movieVideoElement) {
        movieVideoElement.pause();
        movieVideoElement.src = '';
        moviePlayerModal.classList.add('hidden');
      }
